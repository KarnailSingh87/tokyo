...
from tokyox import create_tokyox
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are TOKYO, Tony Stark's AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "TOKYO checks these topics once a day and alerts the user when there is a new development. "
    ...
    },
    {
        ...
        "description": (
            ...
            "close the assistant, say goodbye, or stop Tokyo. "
...
class TokyoLive:
    ...
    def __init__(self, ui: TokyoUI):
        self.ui             = ui
        self._asst_name     = "TOKYO"   # updated each session from config
        self.session              = None
        ...
        # TOKYO-X integration
        self._tokyox = create_tokyox(BASE_DIR)
        self._pairing_code = "-".join(self._tokyox.pair_token[i:i+4] for i in range(0, 16, 4)).upper()
        self.ui.write_log(f"SYS: TOKYO-X online. Pairing code: {self._pairing_code}")

    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask TOKYO to
    ...
    def interrupt(self) -> None:
        """Stop TOKYO mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[TOKYO] ✋ Interrupted — {drained} audio chunks discarded")
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "TOKYO").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "TOKYO"
            _user_name = ""
        ...
        cfg = dict(
            ...
            # window — TOKYO can stay in one conversation for hours
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[TOKYO] 🔧 {name}  {args}")
        self.ui.set_state("THINKING")

        # TOKYO-X permission gate
        if hasattr(self, "_tokyox"):
    ...
    async def _listen_audio(self):
        ...
        try:
            with sd.InputStream(
                ...
                print("[TOKYO] 🎤 Mic stream open")
                while True:
                    await asyncio.sleep(0.1)
        except Exception as e:
            print(f"[TOKYO] ⚠️ Microphone not available ({e}). Voice input disabled, falling back to text/UI input.")
            while True:
                await asyncio.sleep(1)

    async def _receive_audio(self):
        print("[TOKYO] 👂 Recv started")
        out_buf, in_buf = [], []
        ...
        try:
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until TOKYO finishes speaking the answer
            ...
            print(f"[TOKYO] ❌ Recv: {e}")
            traceback.print_exc()
            raise

    async def _play_audio(self):
        print("[TOKYO] 🔊 Play started")
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session:
                # Don't interrupt if user spoke recently or TOKYO is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    print("[TOKYO] Connected.")
                    self.ui.set_state("LISTENING")
                    self.ui.write_log("SYS: TOKYO online.")
                ...
                err_str = str(e)
                print(f"[TOKYO] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    while not self.ui._win._ready:
                        await asyncio.sleep(1)
                    print("[TOKYO] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
            ...
            print(f"[TOKYO] Reconnecting in {delay}s...")
