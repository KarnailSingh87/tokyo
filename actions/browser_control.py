...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".tokyo_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".tokyo_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using TOKYO profile")
                tokyo = str(Path.home() / ".tokyo_profiles" / "firefox_tokyo")
                Path(tokyo).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(tokyo, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".tokyo_profiles" / "safari")
        ...
        # TOKYO otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        tokyo_profile = str(Path.home() / ".tokyo_profiles" / self.browser_name)
        Path(tokyo_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with TOKYO profile: {tokyo_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(tokyo_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with TOKYO profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "tokyo_screenshot.png")
