"""
Drop-in TOKYO plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — TOKYO discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    player: the TokyoUI instance — use player.write_log(f"TOKYO: ...") to log,
    ...
    if player:
        try:
            player.write_log(f"TOKYO: {result_text}")
        except Exception:
            pass
    return result_text
