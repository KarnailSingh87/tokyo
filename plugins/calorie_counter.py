"""
TOKYO plugin — Calorie Counter (webcam vision).

Hold food up to the camera and ask "how many calories is this?" —
TOKYO switches the HUD to the live camera with an animated scan bar,
...
_SCAN_COLOR        = (255, 190, 40)    # TOKYO cyan-blue (BGR)
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    try:
        if stream_sig:
            ...
            view_open = True
        _log("TOKYO: Nutrition scan started.")
    ...
    try:
        ...
        if view_open:
            stream_sig.emit(False)  # back to the TOKYO HUD
    ...
    if data.get("food"):
        ...
        if player and panel:
            try:
                ...
                pass
        _log(f"TOKYO: Nutrition scan complete — {data.get('food')}"
