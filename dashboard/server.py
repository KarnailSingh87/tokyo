"""
dashboard/server.py — TOKYO Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        ...
        Path.home() / "Documents" / "TOKYO Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'TOKYO-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"TOKYO Dashboard Port {port}"
        prog_rule  = "TOKYO Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="tokyo_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until TOKYO is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "tokyo.key").exists() and (certs / "tokyo.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in TOKYO to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('tokyo_token','{tok}');
  sessionStorage.setItem('tokyo_key','{key}');
  localStorage.setItem('tokyo_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to TOKYO…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "tokyo.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "tokyo.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in TOKYO UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
